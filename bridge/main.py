...
def matches_track(filename, artist, title):
    ...
    # Check 3: Relaxed Artist Match (If title matches perfectly, and first letter of artist matches)
    # Useful for Dolphin (D) vs Дельфин (D / Del'fin)
    if t_norm in f_norm or t_uni in f_uni:
        if a_uni and f_uni and a_uni[0] == f_uni[0]:
            return True
...
def create_daily_playlist(library_files=[]):
    ...
    playlist_path = os.path.join(PLAYLISTS_DIR, "Daily Mix.nsp")
    
    # Use "any" rule to combine Daily folder and specific library files
    rules = [
        {"startsWith": {"filepath": "Daily/"}}
    ]
    
    for path in library_files:
        # Ensure path is relative to library root
        clean_path = path.replace("/music/", "") if path.startswith("/music/") else path
        rules.append({"is": {"filepath": clean_path}})

    nsp_data = {
        "name": "Daily Mix",
        "comment": "Generated by Spotify-Soulseek Bridge",
        "any": rules,
        "sort": "random"
    }
    ...
    try:
        with open(playlist_path, 'w', encoding='utf-8') as f:
            json.dump(nsp_data, f, indent=2, ensure_ascii=False)
            
        logger.info(f"Updated Smart Playlist at {playlist_path}")
    except Exception as e:
        logger.error(f"Error saving NSP playlist: {e}")
...
def job_daily_sync():
    ...
    logger.info(f"Loaded {len(library_index)} tracks from library index.")
    ...
    logger.info(f"Found {len(daily_existing)} tracks in Daily folder.")
